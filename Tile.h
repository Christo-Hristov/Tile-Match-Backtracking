...
class Tile {
  public:
     ...
     *
     * Notes:
     * 1. You can assume that each side is a single alphabet character (e.g.,
     * 'A')
     * 2. There will always be either four or five tokens (you do not have to
     *    handle other cases).
     * 3. The fifth token (if it exists) will always be an integer
  ...
  private:
     ...
     * A 4-element array of strings representing the sides of the tile

     * should hold the values "A", "B", "a", "b" in order.
     */
    string sides[NUM_SIDES];

    /* Given a tile, return a vector containing
     * the sides of the tile after the tile has been
     * rotated according to its orientation.  The
     * tile itself should be unchanged.
     */
     Vector<string> orientate();
